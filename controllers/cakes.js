...
${JSON.stringify(req.body)}`)
  try {
    ...
    console.log(toUpdate + " this is toUpdate")
    console.log(req.body)
    if (req.body.cake_flavour)
      toUpdate.cake_flavour = req.body.cake_flavour;
    if (req.body.cost)
      toUpdate.cost = req.body.cost;
    if (req.body.cake_Name)
      toUpdate.cake_Name = req.body.cake_Name;
      
    let result = await toUpdate.save();
    console.log("Sucess " + result)
    res.send(result)

    console.log(toUpdate + "after this is toUpdate")
  } catch (err) {
    res.status(500)
...
exports.cakes_delete_Page = async function (req, res) {
  ...
  try {
    result = await cakes.findById(req.query.id)
    res.render('cakesdelete', {
      title: 'Cakes Delete', toShow:
        result
    });
  }
  catch (err) {
